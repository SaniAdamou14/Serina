...
class EvolutionEngine extends EventEmitter {
  ...
  async initialize() {
    try {
      ...
      // Ensure at least one species exists for simulation
      const existingSpecies = await this.db.getSpeciesBySimulation(this.simulationId);
      if (!existingSpecies || existingSpecies.length === 0) {
        const speciesId = await this.db.createSpecies(this.simulationId, 'Serina Alpha', this.generateRandomTraits());
        console.log('🆕 Created base species with id', speciesId);
      }

      // Create initial population if none exists
      const existingPopulation = await this.db.getPopulation(this.simulationId);
  ...
  async createInitialPopulation() {
    ...
    // Fetch species rows to get IDs
    const dbSpecies = await this.db.getSpeciesBySimulation(this.simulationId);
    const defaultSpeciesId = dbSpecies[0]?.id;
  ...
  generateRandomTraits() {
    const traitNames = [
      'size','speed','intelligence','endurance','aggression','social_tendency',
      'reproduction_rate','lifespan','adaptability','energy_efficiency','sensory_acuity','camouflage'
    ];
    const traits = {};
    traitNames.forEach(t => { traits[t] = Math.random(); });
    return traits;
  }
  ...
  const population = await this.db.getPopulation(this.simulationId);
      ...
      if (now % 10000 < 100) {
        this.generation++;
  await this.db.saveGenerationStats(this.simulationId, this.generation, population);
      }
  ...
  async updateMetabolism(population, deltaTime) {
    const updates = population.map(individual => {
      ...
      const traits = typeof individual.traits === 'string' ? JSON.parse(individual.traits) : (individual.traits || {});
      const metabolicRate = this.calculateMetabolicRate(traits);
  ...
  const baseRate = 1.0;
  const sizeMultiplier = 0.5 + (traits.size || 0.5) * 1.5;
  const speedMultiplier = 0.8 + (traits.speed || 0.5) * 0.4;
  const metabolismMultiplier = 2.0 - (traits.energy_efficiency || 0.5);
  ...
  const traits = typeof individual.traits === 'string' ? JSON.parse(individual.traits) : (individual.traits || {});
  const longevity = traits.lifespan || 0.5;
  ...
  const resistance = (traits.resistance || 0.5);
  ...
  async getSimulationData() {
    try {
  const population = await this.db.getPopulation(this.simulationId);
  const species = await this.db.getSpeciesStats(this.simulationId);
  ...
  async calculateEvolutionStats(population) {
    ...
    traitNames.forEach(trait => {
      const values = alivePopulation.map(ind => {
        const t = typeof ind.traits === 'string' ? JSON.parse(ind.traits) : (ind.traits || {});
        return t[trait] || 0;
      });
    ...
    const averageFitness = alivePopulation.reduce((acc, ind) => acc + this.calculateFitness(ind), 0) / alivePopulation.length;
    
    return {
  totalMutations: await this.db.getTotalMutations(this.simulationId),
  totalReproductions: await this.db.getTotalReproductions(this.simulationId),
  ...
  const traits = typeof individual.traits === 'string' ? JSON.parse(individual.traits) : (individual.traits || {});
  const energyFactor = (individual.energy || 0) / 100;
  const ageFactor = Math.max(0, 1 - (this.generation - (individual.generation || 0)) / 30);
  const traitFactor = ((traits.intelligence || 0.5) + (traits.adaptability || 0.5)) / 2;
