...
router.get('/:id/traits',
  ...
  async (req, res) => {
    try {
      ...
      // No species_traits table; return current species traits and basic stats
      const traits = [{ generation: species.generation_span, ...species.traits }];
      const statistics = Object.entries(species.traits || {}).reduce((acc, [key, val]) => {
        if (typeof val === 'number') {
          acc[key] = { current: val, average: val, min: val, max: val, trend: 0 };
        }
        return acc;
      }, {});
  ...
  const species = await database.getSpecies(simulationId);
      ...
      const summary = {
        ...
        averageFitness: species.length > 0 ? 
          species.reduce((sum, s) => sum + (s.fitness_average || 0), 0) / species.length : 0,
        extinctionRisk: {
...
router.get('/:id/evolution',
  ...
  async (req, res) => {
    try {
      ...
      // Population history derived from individuals by generation
      const populationHistory = await database.query(`
        SELECT generation, COUNT(*) AS population, MIN(created_at) AS first_seen, MAX(updated_at) AS last_seen
        FROM individuals
        WHERE species_id = ?
        GROUP BY generation
