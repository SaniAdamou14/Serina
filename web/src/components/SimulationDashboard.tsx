...
export function SimulationDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'genetics' | 'environment' | 'species'>('overview')
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-gradient">Serina</h1>
                <p className="text-xs text-gray-500">Evolutionary Dashboard</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-3">
            ...
            {activeTab === 'environment' && (
              <div className="space-y-6">
                <EnvironmentView showDetails={true} />
              </div>
            )}
      ...
      <footer className="bg-white border-t border-gray-200 px-4 py-2">
        <div className="max-w-7xl mx-auto flex items-center justify-between text-sm text-gray-500">
          <div className="flex items-center space-x-4">
            <span>Generation: {simulationData?.generation || 0}</span>
            <span>Population: {simulationData?.populationCount || 0}</span>
            <span>Species: {simulationData?.species?.length || 0}</span>
          </div>
          <div className="flex items-center space-x-4">
            <span>FPS: {simulationData?.performance?.frameRate?.toFixed(1) || '0.0'}</span>
            <span>Memory: {simulationData?.performance?.memoryUsage?.toFixed(1) || '0.0'}MB</span>
          </div>
        </div>
      </footer>
    </div>
